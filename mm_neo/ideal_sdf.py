...
if __name__ == "__main__":
    import mm_neo.utils.ros_utils as utils
    import rospy
    from sensor_msgs.msg import PointCloud2
    import time

    rospy.init_node("test_cylinder")
    rate = rospy.Rate(10)

    or_pub = rospy.Publisher("origin", PointCloud2, latch=True)
    f_pub = rospy.Publisher("final", PointCloud2, latch=True)

    # sdf = []
    # for i in range(9):
    #     cylinder = CylinderSDF(1,0.5, [0.15,0.15,0.0])
    #     sdf.append(cylinder)
    #
    # points = torch.rand((10000,3), device = "cuda", requires_grad= True)
    #

    import json

    with open("/media/nmarticorena/DATA/ideal_sdfs/tunnel.json", "r") as f:
        blender = json.load(f)

    sdf = []

    for s in blender["spheres"]:
        sdf.append(SphereSDF(s["radius"], s["position"]))

    # for c in blender["cylinders"]:
    #     sdf.append(BCylinderSDF(c["radius"], c["position"] ))
    #
    while not rospy.is_shutdown():
        rate.sleep()
        points = torch.rand((50000, 3), device="cuda", requires_grad=True)

        points = -10 * points + 5
        ti = time.time()
        d = total(points, sdf)
        grad = gradient(points, d)
        print(f"time {time.time() - ti}")
        # origin_pcd = utils.tensor_to_pointcloud(dpositive)
        # final_pcd = utils.tensor_to_pointcloud(dnegative)
        origin_pcd = utils.tensor_to_pointcloud(points)
        final_pcd = utils.tensor_to_pointcloud(points - d.unsqueeze(-1) * grad)
        or_pub.publish(origin_pcd)
        f_pub.publish(final_pcd)
